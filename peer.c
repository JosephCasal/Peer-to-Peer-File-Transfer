...
//   portno2 = atoi(argv[3]);
   ...
   if(sockfd < 0){ perror("can't open socket"); exit(-1); }
//   printf("create socket...\n");
   ...
   if(connect(sockfd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0)
	...
	printf("Connected to tracker: %s, successful\n", server->h_name);

   //send filenames to tracker
   ...
   if(ret != 0)
      printf("Peer server create thread failed!\n");
   ...
   while(1){
      ...
      if(strcmp(buffer,"list") == 0){
         ...
         if(n < 0){ perror("can't send to server"); continue; }
...
//                 break;
         ...
         if(check == 1){
            printf("error: that file is in your directory!\n");
            continue;
         }
         ...
         if(n < 0){ perror("can't send to server");}
         printf("Connection to tracker %s terminated.\n", server->h_name);
...
void sendfilenames(int sockfd, char* portno){
   ...
   struct dirent *dir;
	...
	//send number of files to tracker
	int count = 0;
   d = opendir(".");
   if (d)
   {
...
void recvFileThread(void * arg){
   ...
   sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
   if(sockfd < 0){ perror("can't open socket"); return; }
//   printf("create socket...\n");
   ...
   if(connect(sockfd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0)
      { perror("can't connect to server"); return; }
   ...
   n = send(info->trackersock, buffer, sizeof(buffer), 0);
   if(n < 0){ perror("can't send to server"); return; }

   //printf("sending filenames with server port: %s\n", info->peerserverport);
   sendfilenames(info->trackersock, info->peerserverport);
...
void serverThread(void * arg){
   ...
   for(;;) {
     ...
     if(newsockfd < 0){ perror("can't accept"); continue; }
      ...
      if(ret != 0)
         printf("send create thread failed!\n");
